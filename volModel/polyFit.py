...
def fit_simple_poly(
    ...
    return {"atm_vol": float(a), "skew": float(b), "curv": float(2 * c), "rmse": rmse, "model": "simple_poly"}
...
def fit_tps(
    ...
    try:
        ...
        rbf = RBFInterpolator(y=k.reshape(-1, 1), d=iv, kernel="thin_plate_spline", smoothing=point_smoothing)
...
def fit_poly(
    k: np.ndarray, iv: np.ndarray, weights: Optional[np.ndarray] = None, method: str = "simple", **kwargs
) -> Dict[str, float]:
    """
...
def fit_tps_slice(
    S: float, K: np.ndarray, T: float, iv: np.ndarray, weights: Optional[np.ndarray] = None, smoothing: float = 1e-2
) -> Dict[str, float]:
    """
